...
### Numeric Content
- Pass numbers directly: `<Badge>{count}</Badge>`
- Automatic truncation: 1234 → "999+"
- Custom maxChars: `<Badge maxChars={2}>{100}</Badge>` → "9+"

### RTL Considerations
- Position names remain logical (top-right = trailing edge)
- Actual positioning flips automatically via `rtl:` classes
- Test with `dir="rtl"` on parent element

### Edge Cases
- Empty badge renders as empty span (use small size for dot)
- Zero values still render (use showBadge={false} to hide)
- Very long text will overflow (use maxChars or truncate content)
- AnimatedBadge with visible={false} remains in DOM (pointer-events-none)
